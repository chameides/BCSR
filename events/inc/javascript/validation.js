...
function validCheckArray(frm,boxes,myMin,error){var cnt = 0;if(document[frm][boxes].length == undefined){if(document[frm][boxes].checked == true || document[frm][boxes].selected == true )cnt++;} else {for(i=0;i<document[frm][boxes].length;i++){if(document[frm][boxes][i].checked===true || document[frm][boxes][i].selected === true)cnt++;}}if(cnt < myMin)return error;return "";}
