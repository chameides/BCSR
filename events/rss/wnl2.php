...
$query = "SELECT e.PkID, e.Title, e.Description, e.StartDate, e.StartTime, e.SeriesID, e.Image, e.EndTime, l.Name, c.CategoryName 
			...
			WHERE e.IsActive = 1 AND e.IsApproved = 1 AND e.StartDate >= '" . cIn(SYSDATE) . "' AND c.IsActive = 1"
			.$lQuery.$cQuery;

$query2 = str_replace(", e.StartDate,", ", MIN(e.StartDate),", $query); //only replace the first instance of e.StartDate from the previous query

$query .= ($hc_cfg[33] == 0) ? " AND SeriesID IS NULL GROUP BY e.PkID UNION " . $query2 . " AND SeriesID IS NOT NULL GROUP BY e.SeriesID" : '';
...
if(hasRows($result)){
	...
	if($hc_cfg[107] > 0){
        ...
        if(isset($row[4]))
        {
            $startTime = date("g:i A",strtotime(stampToDate($row[3].' '.$row[4],"%d %b %Y %H:%M:%S")));
        }
        else{
            $startTime = "";
        }
