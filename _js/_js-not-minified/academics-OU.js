...
$(document).ready(function() {
  ...
  $(".program-item").not("#expanded-content").on("click", function(e) {
    ...
    if ($(this).hasClass('active-program')){
      ...
      var ajax_url = '/_resources/php/academics-listing.php?id=' + $(this).attr('id') + "&xml_feed=" + xml_feed;
      $('#expanded-content').load(ajax_url,function(){
        ...
        thisthing.addClass('active-program');

        // Get insert point and sort
        var rowEndId, expanderSort;
        if (thisthing.hasClass('last-in-row')){
          rowEndId = thisthing.attr('id');
        }
        else{
        ...
        //determine height of revealed content
        var expandedContentHeight1 = $('#expanded-content').height();
        
        //check to see if revealed content is covering up the rows below. 1. delay. 
        setTimeout(function(){
          //2. Compare current height of revealed content to the height when the last sort took place. If they don't match. Sort. 
          var expandedContentHeight2 = $('#expanded-content').height();
          if (expandedContentHeight2 !== expandedContentHeight1) {
            resortGrid($container);
          }
        },500);
        //alert(' / this top: ' + $(this).css('top') + ' / parent top: ' + thisthing.css('top') + ' / (10:21)');
      });
      //MOVED WITHIN AJAX LOAD
      /*
      // Deactivate all previously selected programs
      $(".program-item").removeClass('active-program');
